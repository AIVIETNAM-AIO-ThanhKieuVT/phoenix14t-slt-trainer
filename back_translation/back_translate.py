...
def back_translate(model: SignModel, poses: torch.Tensor
    ...
    sgn = [torch.cat((s, torch.zeros((max_len-s.shape[0], features_dim), device=s.device))) for s in sgn]  # pad
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(num_batches), desc='Running Back Translation', total=num_batches, disable=True):
            _sgn = sgn[start:end]
