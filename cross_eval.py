...
def load_predictions(input_path: Path):
    if input_path.suffix == '.json':
        ...
        pred = torch.load(input_path, weights_only=False, map_location='cpu')
...
def load_gt(gt_path: Path = ''):
    data = torch.load(str(gt_path), weights_only=False, map_location='cpu')
    keys = list(data.keys())
...
if __name__ == '__main__':
    ...
    # filter the gt and the predictions to be the same length
    common_ids = [k for k in ids if k in pose_predictions]
    print(f"Evaluating {len(common_ids)} samples (from {len(pose_predictions)} predictions).")
    
    gt_text = [gt_text[ids.index(id)] for id in common_ids]
    gt_pose = [gt_pose[ids.index(id)] for id in common_ids]
    pose_predictions = [pose_predictions[id] for id in common_ids]
    ids = common_ids
