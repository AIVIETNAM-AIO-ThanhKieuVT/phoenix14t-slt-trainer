...
def pose_acceleration(hyps: list = None, gt_pose: list = None):
    """
    Calculate the mean acceleration of the hand joints.
    """
    def calculate_acceleration(tensor: torch.Tensor) -> float:
        # tensor: [N, 3] (a single joint's trajectory)
        if len(tensor) < 3:
            return 0.0
        # Velocity
        velocity = tensor[1:] - tensor[:-1]
        # Acceleration
        accel = velocity[1:] - velocity[:-1]
        # Mean L2 norm of acceleration
        return torch.norm(accel, dim=1).mean().item()

    hyp_acc_l = torch.mean(torch.tensor([calculate_acceleration(h[..., RELEASE_JOINTS['LWrist'], :]) for h in hyps]))
    hyp_acc_r = torch.mean(torch.tensor([calculate_acceleration(h[..., RELEASE_JOINTS['RWrist'], :]) for h in hyps]))

    gt_acc_l = torch.mean(torch.tensor([calculate_acceleration(h[..., RELEASE_JOINTS['LWrist'], :]) for h in gt_pose]))
    gt_acc_r = torch.mean(torch.tensor([calculate_acceleration(h[..., RELEASE_JOINTS['RWrist'], :]) for h in gt_pose]))

    # Score as ratio of hyp to gt acceleration (should be close to 1.0)
    l_score = hyp_acc_l / (gt_acc_l + 1e-6)
    r_score = hyp_acc_r / (gt_acc_r + 1e-6)
    score = (l_score + r_score) / 2
