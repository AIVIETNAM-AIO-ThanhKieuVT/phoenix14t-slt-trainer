...
class FlashCrossAttention(nn.Module):
    ...
    def forward(
        ...
        if key_padding_mask is not None:
            ...
            attn_mask.masked_fill_(key_padding_mask.bool().unsqueeze(1).unsqueeze(2), float('-inf'))
        ...
        # Robustness: Handle all-masked cases (NaN from softmax(-inf))
...
class ImprovedTransformerLayer(nn.Module):
    ...
    def forward(
        ...
        # FIX: text_mask from tokenizer is torch.long (0/1). Bitwise ~ on long
        # gives -2/-1 (both truthy), masking ALL positions. Must convert to bool first.
