...
# -*- coding: utf-8 -*-
"""
Latent Flow Matcher V11 (1 Apr) — Multi-Scale + Adaptive ODE + Dynamic CFG + CFG Dropout Fix
=============================================================================================
Author: SignFML Research
Date: 1 Apr 2026

Based on latent_flow_matcher_v11_24mar.py. Changes:
1. [BUG FIX] CFG dropout: actually apply cfg_dropout_rate in forward() to train null branch
   (24mar stored cfg_dropout_rate but never used it → null branch untrained → CFG artifacts)
2. All other components identical to V11 24mar:
   - Multi-Scale Flow Matching: U-Net style coarse→fine for sequences
   - Adaptive ODE Solver: Midpoint/RK4 instead of Euler
   - Improved Length Predictor: Duration-per-token regression
   - Dynamic CFG: Timestep-dependent guidance + Rescaled CFG
   - Temporal Attention Bias: Local attention window + relative position bias
"""
...
class LatentFlowMatcherV11(nn.Module):
    ...
    def forward(self, batch: Dict[str, torch.Tensor],
        ...
        # [1APR FIX] CFG dropout: zero out condition for random samples
        # so the null (unconditional) branch is properly trained for CFG inference.
        # V11 24mar stored cfg_dropout_rate but never applied it here.
        if self.training and self.cfg_dropout_rate > 0:
            drop_mask = torch.rand(B, device=condition.device) < self.cfg_dropout_rate
            if drop_mask.any():
    ...
    def _velocity_step(self, x: torch.Tensor, t_val: float, condition: torch.Tensor,
        ...
        # CFG (Interval CFG: skip at edges to speed up 30% and prevent jitter)
        use_interval_cfg = False
        if cfg_scale != 1.0 and null_cond is not None and (not use_interval_cfg or (0.2 <= t_val <= 0.8)):
