...
# -*- coding: utf-8 -*-
"""
Latent Flow Matcher V2 - SOTA Improvements
===========================================
Author: SignFML Research
Date: 21 Jan 2026

Improvements over V1:
1. Uses FlowMatchingBlockV2 (RoPE, AdaLN-Zero, Flash Attention, LayerScale)
2. Uses TransformerPriorV2 (Cross-attention to text)
3. CFG-Zero* sampling for better guidance
4. SafeFlowMatchingLoss for numerical stability
5. Better NaN handling throughout
"""
...
class LengthPredictor(nn.Module):
    ...
    ✅ FIX: init_mean_length is now interpreted as RAW FRAMES and converted
    to log-space internally for bias init. Previously 120.0 was used directly
    as bias → Softplus(120)≈120 → clamp(max=6.25) → always predicted max length.
    """
    ...
    def __init__(self, input_dim: int, hidden_dim: int = 256, init_mean_length: float = 120.0,
        ...
        self.attn_dropout = nn.Dropout(dropout)  # ✅ FIX: Regularize pooled features
        self.net = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(dropout),  # ✅ FIX: Add second dropout (was missing → overfitting)
            nn.Linear(hidden_dim, 1),
            nn.Softplus()
        )
        # ✅ FIX: Convert raw frames → log-space for bias init
        # Softplus(x) ≈ x for x > 5, so bias ≈ log(init_mean_length) makes
        # the initial prediction exp(log(init_mean_length)) = init_mean_length frames
    ...
    def forward(self, text_features: torch.Tensor, 
        ...
        pooled = self.attn_dropout(pooled)  # ✅ FIX: Dropout on pooled features
        pred_length = self.net(pooled)
        # ✅ Log-space clamping (log(1.0)=0.0, log(512.0)≈6.24)
...
class MaskProjector(nn.Module):
    ...
    def __init__(self, hidden_dim: int):
        super().__init__()
        # ✅ FIX: bias=False so mask=0 → output=0 (no leaked bias when no hand/face)
        self.proj = nn.Linear(1, hidden_dim, bias=False)
    
    def forward(self, mask: torch.Tensor) -> torch.Tensor:
        # ✅ FIX: Auto-unsqueeze if mask is 2D [B, T] → [B, T, 1]
        if mask.dim() == 2:
            mask = mask.unsqueeze(-1)
        return self.proj(mask.float())


class LatentFlowMatcherV2(nn.Module):
    def __init__(
        self,
        latent_dim: int = 256,
        text_encoder_name: str = 'bert-base-multilingual-cased',
        hidden_dim: int = 512,
    ...
    def encode_text(self, text_tokens: torch.Tensor, 
        ...
        text_features = self.text_proj(bert_features)
        text_mask = attention_mask.bool()
        return text_features, text_mask, bert_features
    ...
    def _train_forward(
        ...
        # ✅ FIX Bug #12: Use log-space target for length prediction
        target_len = torch.log(batch['seq_lengths'].float() + 1e-6)
        # Beta=0.1 is standard for values in the [0, 6] range
        length_loss = F.smooth_l1_loss(pred_length, target_len, beta=0.1)
        # MAE in raw frames (for logging)
        length_mae = (torch.exp(pred_length) - batch['seq_lengths'].float()).abs().mean()
    ...
    def sample_cfg_zero_star(
        ...
        if target_length is not None:
            ...
            # ✅ FIX: Exponentiate pred_length (it is predicted in log-space)
        ...
        if conditions is not None:
            ...
            if conditions.get('hand_mask') is not None:
                ...
                # ✅ FIX: Ensure 3D before pad (2D pad would pad wrong dimension)
                if m.dim() == 2:
                    m = m.unsqueeze(-1)
                if m.shape[1] != T:
                    # ✅ FIX: Pad with value=1.0 (not 0) to maintain hand detail
                    # until end of sequence. Padding 0 = "no hands" → blurry hands.
                    m = F.pad(m[:, :T], (0, 0, 0, max(0, T - m.shape[1])), value=1.0)
                mask_features += self.hand_projector(m)
            if conditions.get('face_mask') is not None:
                m = conditions['face_mask']
                # ✅ FIX: Ensure 3D before pad
                if m.dim() == 2:
                    m = m.unsqueeze(-1)
                if m.shape[1] != T:
                    # ✅ FIX: Pad with value=1.0
                    m = F.pad(m[:, :T], (0, 0, 0, max(0, T - m.shape[1])), value=1.0)
...
if __name__ == "__main__":
    print("Testing LatentFlowMatcherV2 Components")