...
# -*- coding: utf-8 -*-
"""
Latent Flow Matcher V4-Trans (2 Mar) — mBART-50 + Transformer Length Predictor
===============================================================================
Author: SignFML Research
Date: 2 Mar 2026

Changes from V4 (27 Feb):
    - Length Predictor: MLP (3-layer + attention pooling)
                      → Transformer (2-layer TransformerEncoder + attention pooling)
    - Soft-sigmoid clamping for length output (more stable than hard clamp)

All other components remain identical to V4:
    - mBART-50 Large text encoder (frozen)
    - FlowMatchingBlockV2
    - TransformerPriorV2
    - CFG-Zero* sampling
"""
...
# Import V2 components (same as V4)
try:
...
class TransformerLengthPredictor(nn.Module):
    ...
    Replaces the MLP-based LengthPredictor in V4.
    Attends to all text tokens before pooling → better captures
    linguistic length cues (e.g. long compound words, enumeration).

    Output is in LOG-SPACE via soft-sigmoid:
        pred = log_min + (log_max - log_min) * sigmoid(raw - offset)
    ...
    def reset_weights(self):
        """Full reset of all weights — useful if loading from V4 checkpoint."""
    ...
    def forward(self, text_features: torch.Tensor,
        ...
        """
        Args:
            text_features: [B, T, input_dim] — raw mBART features
            mask: [B, T] bool, True = valid token
        Returns:
            pred_log_length: [B] — log(num_frames), soft-clamped to [log(10), log(512)]
        """
        ...
        if mask is not None:
            src_key_padding_mask = ~mask  # invert: True = ignore padding
        ...
        # Predict & soft-clamp to (log(~2.7), log(~665)) ≈ valid frame range
        raw = self.head(pooled).squeeze(-1)  # [B]
        min_log, max_log = 1.0, 6.5
        pred_log_length = min_log + (max_log - min_log) * torch.sigmoid(raw - self.sigmoid_offset)
...
class LatentFlowMatcherV4Trans(nn.Module):
    """
    V4-Trans: mBART-50 Large encoder + Transformer Length Predictor.

    Identical to V4 (27 Feb) except LengthPredictor (MLP) is replaced
    with TransformerLengthPredictor.

    Weight compatibility with V4 checkpoint:
        - text_encoder:      ✅ fully compatible
        - text_proj:         ✅ fully compatible
        - flow_block:        ✅ fully compatible
        - ssm_prior:         ✅ fully compatible
        - length_predictor:  ❌ new architecture → auto reinit on load (strict=False)
    """
    def __init__(
        self,
        latent_dim: int = 256,
        ...
        # Transformer length predictor specific
        lp_num_layers: int = 2,
        lp_num_heads: int = 4,
        lp_dropout: float = 0.2,
    ):
        super().__init__()
        ...
        # mBART-50 encoder (frozen) — same as V4
        print(f"🆕 V4-Trans: Loading mBART-50 encoder from: {text_encoder_name}")
        ...
        # 🆕 Transformer Length Predictor (replaces MLP)
        self.length_predictor = TransformerLengthPredictor(
            input_dim=text_dim,
            hidden_dim=hidden_dim,
            num_layers=lp_num_layers,
            num_heads=lp_num_heads,
            dropout=lp_dropout,
        )
        print(f"  📏 Length Predictor: Transformer ({lp_num_layers}L, {lp_num_heads}H)")
    ...
    def encode_text(self, text_tokens: torch.Tensor,
        ...
        raw_features = outputs.last_hidden_state  # [B, L, 1024]
...
if __name__ == "__main__":
    print("Testing LatentFlowMatcherV4Trans (mBART-50 + Transformer Length Predictor)")
